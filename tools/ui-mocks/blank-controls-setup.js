async (page) => {
  ...
  page.on('pageerror', error => fixture.clientErrors.push(error.message));
  const revisions = doc => Object.fromEntries(doc.pages.map(number => [String(number), doc.page_details[number].revision]));
  const snapshotMatches = (doc, body) => body.order_revision === doc.order_revision && body.page_revisions &&
    Object.keys(body.page_revisions).length === doc.pages.length && doc.pages.every(number => body.page_revisions[number] === doc.page_details[number].revision);
  ...
  await page.route('**/*', async route => {
    ...
    if (path === '/__mock__') {
      if (method === 'POST') {
        ...
        for (const key of ['active_id', 'scanner', 'failNext', 'listDelay', 'createDelay', 'candidates', 'analysisErrors', 'analysisDelay', 'staleAnalysis', 'staleDeleteOnce']) {
    ...
    const match = path.match(/^\/scan\/([^/]+)\/(status|continue|rename|name-lock|page|original|pdf|delete|reorder|rescan|blank-analysis|delete-selected|blank-undo)(?:\/(\d+))?$/);
    ...
    if (['blank-analysis','delete-selected','blank-undo'].includes(operation) && fixture.active_id) {
      return respond(route,{error:'扫描任务进行中',active_id:fixture.active_id},409);
    }
    if (operation === 'blank-analysis') {
      const result={document_id:doc.id,order_revision:doc.order_revision,page_revisions:revisions(doc),
        candidates:fixture.candidates.filter(number=>doc.pages.includes(number)).map(number=>({page:number,position:doc.pages.indexOf(number)+1,revision:doc.page_details[number].revision})),
        examined:doc.pages.length,errors:fixture.analysisErrors.filter(number=>doc.pages.includes(number)).map(page=>({page,reason:'mock image unavailable'}))};
      if (fixture.staleAnalysis) result.order_revision--;
      if (fixture.analysisDelay) await page.waitForTimeout(fixture.analysisDelay);
      return respond(route,result);
    }
    if (operation === 'delete-selected') {
      if (fixture.staleDeleteOnce) {
        fixture.staleDeleteOnce=false;
        doc.page_details[doc.pages[0]].revision++;
        doc.updated_at=new Date().toISOString();
      }
      if (!snapshotMatches(doc,body)) return respond(route,{error:'页面已变化，请刷新后重新选择'},409);
      if (!Array.isArray(body.pages) || !body.pages.length || body.pages.some(number=>!doc.pages.includes(number))) return respond(route,{error:'请选择有效页面'},400);
      const removed=doc.pages.filter(number=>body.pages.includes(number));
      const cleanup_id='cleanup-'+fixture.nextId++;
      fixture.undoHistory={cleanup_id,pages:doc.pages.slice(),removed};
      doc.pages=doc.pages.filter(number=>!body.pages.includes(number));
      doc.order_revision++;
      doc.blank_undo={cleanup_id,count:removed.length};
      doc.updated_at=new Date().toISOString();
      fixture.undoHistory.after={order_revision:doc.order_revision,page_revisions:revisions(doc)};
      return respond(route,{document:doc,removed,cleanup_id});
    }
    if (operation === 'blank-undo') {
      const undo=fixture.undoHistory;
      if (!undo || body.cleanup_id!==undo.cleanup_id || !snapshotMatches(doc,undo.after)) return respond(route,{error:'页面已变化，无法撤销这次删除'},409);
      doc.pages=undo.pages.slice();
      doc.order_revision++;
      doc.blank_undo=null;
      doc.updated_at=new Date().toISOString();
      fixture.undoHistory=null;
      return respond(route,{document:doc,restored:undo.removed});
    }
