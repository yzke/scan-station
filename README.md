...
- 网页优先选择 **300 DPI**；设备报告支持后可使用 150、200、300 DPI，单双面选项也以设备报告为准。未确认支持的分辨率和双面扫描会被拒绝。
- 开始扫描前先确认扫描电脑在线。读取心跳文件本身就是对该电脑的存活检查，因此“电脑未开机或网络不通”和“电脑在线但扫描代理没有运行”会分别提示，并说明该做什么。一次扫描请求会一直占用扫描仪，直到代理报出终止状态，所以两者任一不成立时都会拒绝开扫，而不是把请求排进队列等待。
- 收到终止状态前可以“强制终止本批”。终止会先撤回扫描电脑上排队的请求，再释放占用：已收到的页面、页序和 PDF 全部保留，文件仍留在记录里，可以稍后“继续扫描”从下一页接着扫，也可以删除。扫描电脑当时不可达时无法撤回，服务会在恢复连接后自动重试，避免这份请求以后被意外扫出来；若代理已经在走纸，已经进送纸器的纸张仍会被扫完并留在扫描电脑的 `out` 目录，本机不会补收这些页面。
